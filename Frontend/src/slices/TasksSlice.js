...
export const deleteTasksFromServer = createAsyncThunk(
    ...
    async (task,{rejectWithValue}) => {
        ...
        const response = await fetch(`${api_url}/${task._id}`, options);
        if(response.ok) {
            const jsonResponse = await response.json();
            return jsonResponse;
        } else {
...
const TasksSlice = createSlice({
    ...
    reducers: {
        ...
        removeTaskFromList: (state, action) => {
            state.tasksList = state.tasksList.filter((task) => task._id !== action.payload.id)
        },
        updateTaskInList: (state, action) => {
            state.tasksList = state.tasksList.map((task) => task._id === action.payload.id ? action.payload : task)
        },
